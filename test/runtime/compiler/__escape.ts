...
describe('compiler/Escape', () => {
  ...
  it('Should escape Kind', () => {
    TypeRegistry.Set("x', 0, value), globalThis.__global_state__ = true, custom('x", () => true)
    global.__global_state__ = false
    const kind = JSON.parse(String.raw`{"kind":"x', 0, value), globalThis.__global_state__ = true, custom('x"}`).kind
